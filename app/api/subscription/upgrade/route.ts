...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!billingMode || !['MONTHLY', 'ONE_TIME'].includes(billingMode)) {
            return NextResponse.json({ success: false, error: 'INVALID_BILLING_MODE' }, { status: 400 })
        }
        ...
        if (
            ...
            subscription.billingMode === 'MONTHLY' &&
            billingMode === 'MONTHLY' &&
            ...
            await stripe.subscriptions.update(subscription.providerSubscriptionId, {
                ...
                metadata: { ...stripeSub.metadata, plan },
            })
            // webhook (customer.subscription.updated) will sync plan + period locally
            return NextResponse.json({ success: true, redirect: '/dashboard/subscription?updated=true' })
        }

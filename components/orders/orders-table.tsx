        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
                ...
                <p className="text-sm text-muted-foreground font-medium">
                    {t('orders.status.empty', { defaultValue: 'No orders found' })}
                </p>
            </div>
        )
    }
                ...
                {orders.map((o) => (
                    <div key={o.id} className="p-4 space-y-2">
                        ...
                        <div className="flex items-center justify-between pt-1">
                            <span className="font-medium">{Number(o.totalPrice).toLocaleString()} DZD</span>
                            {o.status === 'PENDING_REVIEW' && (
                                <button
                                    onClick={() => onConfirm(o.id)}
                                    disabled={confirmingId === o.id}
                                    className="text-xs px-3 py-1.5 rounded-lg bg-secondary/10 border border-secondary/30 text-secondary hover:bg-secondary/20 disabled:opacity-50 transition"
                                >
                                    {confirmingId === o.id ? <Loader2 className="h-3 w-3 animate-spin" /> : t('orders.confirm')}
                                </button>
                            )}
                        </div>
                    </div>
